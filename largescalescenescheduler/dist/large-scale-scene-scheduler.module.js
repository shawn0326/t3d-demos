import { EventDispatcher, Box3, Vector3, Matrix4, Frustum } from 't3d';
...
class CustomizeBox3 extends Box3 {
...
class OctreeSelector extends Selector {
	...
	subdivide() {
		...
		const arr = [
			...
			[_vec3_1$1.x, 0, 0],
			[0, 0, _vec3_1$1.z],
			[_vec3_1$1.x, 0, _vec3_1$1.z],
			[0, _vec3_1$1.y, 0],
			[_vec3_1$1.x, _vec3_1$1.y, 0],
			[0, _vec3_1$1.y, _vec3_1$1.z],
			[_vec3_1$1.x, _vec3_1$1.y, _vec3_1$1.z]
		];
		for (let i = 0; i < 8; i++) {
...
class ReplaceScheduler extends Scheduler {
	...
	_updateFrustum(camera, renderStates) {
		...
		frustum.setFromMatrix(
			_mat4_1$1.multiplyMatrices(
				camera.projectionMatrix,
				camera.viewMatrix// camera.matrixWorldInverse
			)
		);

		const farPlane = frustum.planes[4], nearPlane = frustum.planes[5], worldMatrix = camera.worldMatrix, near = renderStates.camera.near;
...
export { CustomizeBox3, LargeScaleInstancedManager, OctreeSelector, ReplaceScheduler, Scheduler, Selector };
